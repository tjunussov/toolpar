...
<binfo>
		...
		<div  id="B{branch_id}" branch="{branch_id}" prefix="{code}">
			<xsl:attribute name="class">
				branch branch-mini <xsl:if test="number(waiting_stat/max_time_waiting) &gt;= $LIMIT_MAXTIME_WAITING or number(serving_stat/max_time_serving) &gt;= $LIMIT_MAXTIME_SERVING">max </xsl:if> <xsl:value-of select="$block_sizing"/>
			</xsl:attribute>
            ...
            <v class="diff"><xsl:value-of select="format-number(number(waiting_stat/total_cnt_is_waiting_now),'0')"/></v><xsl:if test="waiting_stat/total_cnt_exceed_waiting!='' and waiting_stat/max_time_waiting!=''">/<v><xsl:if test="number(waiting_stat/max_time_waiting) &gt;= $LIMIT_MAXTIME_WAITING"><xsl:attribute name="class">max blink</xsl:attribute></xsl:if>
